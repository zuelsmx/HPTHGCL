...
torch.serialization.add_safe_globals([torch_geometric.data.storage.BaseStorage, torch_geometric.data.storage.NodeStorage, torch_geometric.data.storage.EdgeStorage])

from model import HPTHGCLModel, Encoder
...
def main():
    ...
    encoder = Encoder(in_dim=num_feat, hid_dim=params['hid_dim'], num_nodes=num_nodes, num_layers=params['num_layers'])            
    model = HPTHGCLModel(encoder=encoder, hid_dim=params['hid_dim'], num_relations=num_relations,
                         tau=params['tau'], alpha=params['alpha'],
                         moving_average_decay=params['moving_average_decay']).to(device)
